...
content = re.sub(r'class ([A-Z][a-zA-Z0-9_]*):', r'class \1(BaseModel):\n    model_config = ConfigDict(strict=True)', content)
...
# Special cases
content = content.replace("def _construct(dataclass_type: type[Any], payload: dict[str, Any] | None) -> Any:\n    return dataclass_type(**(payload or {}))", "def _construct(dataclass_type: type[Any], payload: dict[str, Any] | None) -> Any:\n    return dataclass_type.model_validate(payload or {})")

content = content.replace("def to_dict(self) -> dict[str, Any]:\n        return asdict(self)", "def to_dict(self) -> dict[str, Any]:\n        return self.model_dump()")

# Exception should not inherit from BaseModel
content = content.replace("class ConfigValidationError(BaseModel):\n    model_config = ConfigDict(strict=True)\n    pass", "class ConfigValidationError(ValueError):\n    pass")
content = content.replace("class ConfigValidationError(BaseModel):\n    model_config = ConfigDict(strict=True)", "class ConfigValidationError(ValueError):")
