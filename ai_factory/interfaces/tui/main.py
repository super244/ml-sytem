...
class TUIInterface:
    ...
    def __init__(self, repo_root: Path | None = None, artifacts_dir: Path | None = None):
        ...
        self.container = build_platform_container(repo_root=self.repo_root, artifacts_dir=self.artifacts_dir)

    def run(self, refresh_seconds: float = 2.0) -> None:
        """Run the TUI interface."""
        # Delegate to existing TUI implementation
        legacy_run_tui(
            repo_root=str(self.repo_root), artifacts_dir=str(self.artifacts_dir), refresh_seconds=refresh_seconds
        )
