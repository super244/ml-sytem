    ...
    def compute_loss(
        ...
        try:
            ...
            if teacher_logits.shape != student_logits.shape:
                ...
                labels_truncated = labels[:, :min_seq_len]
            else:
                student_logits_truncated = student_logits
                labels_truncated = labels
            
            # Create soft labels from teacher with temperature
            teacher_probs = F.softmax(teacher_logits / self.teacher_config.temperature, dim=-1)
            ...
            # Combine losses
            total_loss = (
                self.teacher_config.alpha * distill_loss +
                (1 - self.teacher_config.alpha) * student_loss
            )
...
def create_teacher_student_trainer(
    ...
    tokenizer=None,
    data_collator=None,
    teacher_config: Optional[TeacherStudentConfig] = None,
    **kwargs
