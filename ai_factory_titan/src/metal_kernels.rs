...
fn compile_metal_library(device: &Device) -> Result<Library> {
    let source = include_str!(concat!(env!("CARGO_MANIFEST_DIR"), "/src/metal/shaders.metal"));
    ...
    unsafe {
        let raw_options: *mut metal::MTLCompileOptions = options.as_ptr();
        let fast_math_enabled: objc::runtime::BOOL = objc::runtime::YES;
        let _: () = msg_send![raw_options, setFastMathEnabled: fast_math_enabled];
    }
...
pub fn matmul_f32_metal(
    ...
    if a.len() != m * k || b.len() != k * n {
        return Err(anyhow!("Matrix dimension mismatch: a={}, expected={}, b={}, expected={}", 
            a.len(), m * k, b.len(), k * n));
    }
...
pub fn matmul_q4_0_metal(
    ...
    let a_buffer = allocate_unified_buffer(device, a.len() * std::mem::size_of::<f32>());
    let q_buffer = allocate_unified_buffer(device, q_weights.len() * std::mem::size_of::<crate::tensor::BlockQ4_0>());
...
fn estimate_gpu_cores(name: &str) -> usize {
    ...
    if name.contains("Max") {
        if name.contains("M5") { 40 } else if name.contains("M4") { 40 } else { 32 }
    } else if name.contains("Pro") {
        if name.contains("M5") { 24 } else if name.contains("M4") { 20 } else { 18 }
    } else {
        // Base models
        if name.contains("M5") { 10 } else if name.contains("M4") { 10 } else if name.contains("M3") { 10 } else { 8 }
    }
}
