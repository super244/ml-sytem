...
def test_collect_metrics_for_training_instance(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    (run_dir / "manifests" / "run_manifest.json").write_text(
        '{"run_id": "finetune-20260101-000000", "run_name": "finetune", '
    ...
    snapshot = {"resolved_subsystem_config": {"run_name": "finetune", "training": {"artifacts_dir": str(tmp_path)}}}
...
def test_collect_metrics_for_evaluation_instance(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    (output_dir / "summary.json").write_text(
        '{"primary": {"accuracy": 0.88, "parse_rate": 0.75, '
        '"verifier_agreement_rate": 0.7, "no_answer_rate": 0.05, '
        '"avg_latency_s": 1.2}}'
    )
