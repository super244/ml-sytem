...
import { automlRuns } from '@/data/mockData';
...
const bestRun = automlRuns.filter(r => r.status === 'promoted').sort((a, b) => a.loss - b.loss)[0];
...
const AutoMLExplorer = () => {
  ...
  const sortedRuns = [...automlRuns].sort((a, b) => {
          ...
          <div className="grid grid-cols-1 lg:grid-cols-5 gap-4">
            ...
            <div className="lg:col-span-2">
              ...
              <GlassCard className="space-y-3">
                <DetailRow label="Search ID" value="search-bayesian-001" />
                <DetailRow label="Strategy" value="Bayesian Optimization" />
                ...
                <button className="w-full text-xs font-mono px-3 py-2 rounded-lg bg-neon-green/20 border border-neon-green/30 text-neon-green hover:bg-neon-green/30 transition-colors">
                  → Promote Now
                </button>
              </GlassCard>
            </div>
              ...
              <table className="w-full text-xs font-mono">
                ...
                <tbody>
                  {sortedRuns.slice(0, 15).map(run => (
                    <tr key={run.id} className="border-b border-border/50 hover:bg-raised/50 transition-colors">
