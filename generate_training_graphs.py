...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Generate standard training-run analysis graphs (7 figures)."
    )
    ...
    parser.add_argument(
        ...
        default=None,
        metavar="DIR",
        help="Where to write PNGs (default: <run-dir>/graphs).",
    )
    args = parser.parse_args()
    run_dir = resolve_run_dir(args)
    output_dir = resolve_output_dir(args, fallback=run_dir / "graphs")
    ...
    df = bundle.metrics_df

    _generate_figures(dataset_data, model_data, final_metrics, df, output_dir)
