  ...
  return (
    <section className="panel aside-section">
      <div className="model-chip-header">
        ...
        <span className="status-pill" style={{ borderColor: tone(autonomy.status), color: tone(autonomy.status) }}>
      ...
      {!compact ? (
        <div className="workspace-section-grid" style={{ marginTop: "1rem" }}>
          <div className="resource-list">
            {stages.map((stage) => (
              <a key={stage.id} href={stage.href} className="resource-card" style={{ borderTop: `3px solid ${tone(stage.status)}` }}>
                <div className="model-chip-header">
                  <strong>{stage.title}</strong>
                  <span style={{ color: tone(stage.status), textTransform: "capitalize" }}>{stage.status}</span>
                </div>
                <p>{stage.headline}</p>
                <p className="control-label">{stage.detail}</p>
                {stage.metric_label ? (
                  <div className="badge-row">
                    <span className="status-pill">{stage.metric_label}: {stage.metric_value ?? "n/a"}</span>
                  </div>
                ) : null}
              </a>
            ))}
          ...
          <div className="resource-list">
            {actions.map((action) => (
              <a key={action.id} href={action.href} className="resource-card" style={{ borderTop: `3px solid ${action.blocking ? "var(--danger)" : tone(autonomy.status)}` }}>
                <div className="model-chip-header">
                  <strong>{action.title}</strong>
                  <span style={{ textTransform: "capitalize" }}>{action.category}</span>
                </div>
                <p>{action.detail}</p>
                {action.command ? <code style={{ fontSize: "0.8rem", opacity: 0.8 }}>{action.command}</code> : null}
              </a>
            ))}
          </div>
        </div>
