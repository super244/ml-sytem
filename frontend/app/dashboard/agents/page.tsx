    ...
    <div className="dashboard-content">
      <div className="dash-page-header panel">
        <div>
          ...
          <p className="dash-page-desc">
            Inspect the active swarm, adjust agent roles, and watch orchestration activity as it
            happens.
          </p>
        </div>
      </div>
      ...
      {!deployEnabled && (
        <div className="panel state-panel">
          Agent registry and logs are live, but deploying simulated swarm agents is disabled outside
          demo mode.
        </div>
      )}
          ...
          <div className="model-chip-header">
            <div>
              ...
              <p className="control-label">
                Queue pressure, running tasks, and swarm coverage are synchronized against the same
                autonomy snapshot.
              </p>
            </div>
              ...
              >
                ...
                <div className="badge-row">
                  ...
                  <span className="status-pill">
                    {coverage.resource_classes.join(' / ') || 'control'}
                  </span>
                </div>
              </article>
            ))}
      ...
      <div className="workspace-section-grid">
                ...
                >
                  ...
                  >
                    ...
                    <div style={{ display: 'flex', alignItems: 'center', gap: '0.5rem' }}>
                      <span
                        className={`monitor-status-dot status-${agent.status === 'active' ? 'running' : 'idle'}`}
                      />
                      <span
                        style={{ fontSize: '0.8rem', textTransform: 'capitalize', opacity: 0.8 }}
                      >
                        {agent.status}
                      </span>
                    </div>
        ...
        <div className="panel aside-section" style={{ flex: 1 }}>
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
            ...
            >
              <span className="monitor-status-dot status-running" />{' '}
              {simulationEnabled ? 'Simulated stream' : 'Persisted log stream'}
            </div>
          </div>
          ...
          >
            {logs.length === 0 ? (
              <div style={{ color: '#8b949e' }}>No agent log events have been recorded yet.</div>
            ) : (
              logs.map((log) => {
                ...
                const isAccent =
                  log.message.includes('Optimization') || log.message.includes('Red-Team');
                return (
                  <div key={log.timestamp} style={{ display: 'flex', gap: '1rem' }}>
                    <span style={{ color: '#8b949e', minWidth: '80px' }}>[{timeStr}]</span>
                    <span style={{ color: isAccent ? 'var(--accent)' : 'inherit' }}>
                      {log.message}
                    </span>
                  </div>
                );
        ...
        >
          ...
          >
            ...
            <div style={{ display: 'flex', gap: '0.75rem', marginTop: '0.5rem' }}>
              ...
              <button className="ghost-button" onClick={() => setShowDeployModal(false)}>
                Cancel
              </button>
            </div>
          </div>
          ...
          >
            ...
            <div className="input-group">
              ...
              <input
                ...
                onChange={(e) =>
                  setEditingAgent((current) => ({ ...current, name: e.target.value }))
                }
              />
            </div>
            ...
            <div className="input-group">
              ...
              <textarea
                ...
                onChange={(e) =>
                  setEditingAgent((current) => ({ ...current, role: e.target.value }))
                }
                style={{ height: '88px' }}
              />
            </div>
            ...
            <div className="input-group">
              ...
              <input
                ...
                onChange={(e) =>
                  setEditingAgent((current) => ({ ...current, model: e.target.value }))
                }
              />
            </div>
              ...
              <select
                ...
                onChange={(e) =>
                  setEditingAgent((current) => ({
                    ...current,
                    status: e.target.value as AgentSwarmStatus['status'],
                  }))
                }
              >
