...
export default function DatasetsPage() {
  ...
  useEffect(() => {
    getTelemetryBacklog()
      ...
      .catch((nextError) => {
        setError(
          nextError instanceof Error ? nextError.message : 'Failed to load telemetry backlog.',
        );
      })
  ...
  async function handleTelemetryAction(recordId: string, action: 'promote' | 'discard') {
    ...
    try {
      ...
      setError(
        nextError instanceof Error ? nextError.message : `Failed to ${action} telemetry record.`,
      );
    } finally {
      setActioningId(null);
    }
  ...
  async function handleSynthesize() {
    ...
    try {
      ...
      setError(
        nextError instanceof Error ? nextError.message : 'Failed to dispatch synthesis job.',
      );
    } finally {
  ...
  const synthesisProgress = synthJob
    ? Math.min(
        100,
        Math.round((synthJob.completed_rows / Math.max(synthJob.num_variants, 1)) * 100),
      )
    : 0;
      ...
      <div className="dash-page-header panel">
        <div>
          ...
          <p className="dash-page-desc">
            Turn production failures into curated training assets and launch new synthetic data jobs
      ...
      <div className="workspace-section-grid">
        ...
        <div className="panel aside-section">
          <div>
            ...
            <p className="control-label">
              Review flagged failure cases, then promote them into the curation queue or discard
              them cleanly.
            </p>
          </div>
          ...
          {loading ? (
            <div className="dash-loading">
              <span>⟳</span> Loading records...
            </div>
          ) : telemetry.length === 0 ? (
            <div className="dash-empty" style={{ padding: '3rem 1rem' }}>
              <span className="dash-empty-icon" style={{ fontSize: '2rem', opacity: 0.5 }}>
                ✓
              </span>
              ...
              {telemetry.map((t) => (
                <div key={t.id} className="resource-card">
                  ...
                  <div style={{ marginBottom: '1rem', opacity: 0.7 }}>
                    ...
                    <div
                      style={{ fontSize: '0.85rem', marginTop: '0.25rem', whiteSpace: 'pre-wrap' }}
                    >
                      {t.assistant_output}
                    </div>
                  </div>
        ...
        <div className="panel aside-section">
          <div>
            ...
            <p className="control-label">
              Launch a synthesis job and track row-level progress while it runs.
            </p>
          </div>
          ...
          <div className="input-group">
            <label className="control-label" htmlFor="variants-range">
              Dataset Size (Rows): {numVariants}
            </label>
            <input
              id="variants-range"
              type="range"
          ...
          <div className="input-group" style={{ flex: 1 }}>
            <label className="control-label" htmlFor="seed-prompt">
              Seed Prompt Details
            </label>
            <textarea
              id="seed-prompt"
          ...
          {synthJob ? (
            ...
            >
              ...
              <p style={{ fontSize: '0.85rem', opacity: 0.8, margin: 0 }}>
                Job ID: <code>{synthJob.job_id}</code>
              </p>
              <p style={{ fontSize: '0.85rem', opacity: 0.8, margin: 0 }}>
