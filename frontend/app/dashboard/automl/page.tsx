    ...
    async function refresh(initialLoad: boolean = false) {
      ...
      try {
        ...
        if (sweepsResponse.status === 'fulfilled') {
          ...
          setError(
            sweepsResponse.reason instanceof Error
              ? sweepsResponse.reason.message
    ...
    <div className="dashboard-content">
      <div className="dash-page-header panel">
        <div>
          ...
          <p className="dash-page-desc">
            Launch Bayesian, grid, or random hyperparameter sweeps across your models. The studio
            surfaces optimal configurations automatically so you never hand-tune again.
          </p>
        </div>
      </div>
      ...
      {!writeEnabled && (
        <div className="panel state-panel">
          AutoML inventory is live in production, but launching simulated sweeps is disabled outside
          demo mode.
        </div>
      )}
      {!demoMode && availableModels.length === 0 && (
        <div className="panel state-panel">
          No live model registry entries are available yet, so new sweep launch inputs stay disabled
          until inventory appears.
        </div>
      )}
        ...
        <section className="panel aside-section">
          <div className="model-chip-header">
            <div>
              ...
              <p className="control-label">
                Sweep fan-out is constrained by current cluster capacity, queue pressure, and
                lineage backlog.
              </p>
            </div>
          ...
          <div className="badge-row" style={{ marginTop: '0.75rem' }}>
            ...
            <span className="status-pill">
              schedulable trials {mission.autonomy.capacity.schedulable_trials}
            </span>
            <span className="status-pill">
              suggested parallelism {mission.autonomy.capacity.suggested_parallelism}
            </span>
      ...
      >
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: '1.5rem' }}>
          <div className="panel aside-section">
            ...
            >
              <div className="input-group">
                <label className="control-label" htmlFor="sweep-name">
                  Sweep Name
                </label>
                <input
                  id="sweep-name"
              ...
              <div className="input-group">
                <label className="control-label" htmlFor="base-model">
                  Base Model
                </label>
                <select
                  id="base-model"
              ...
              <div className="input-group">
                <label className="control-label" htmlFor="strategy">
                  Search Strategy
                </label>
                <select
                  id="strategy"
              ...
              <div className="input-group">
                <label className="control-label" htmlFor="num-trials">
                  Trials: {numTrials}
                </label>
                <input
                  id="num-trials"
          ...
          <div className="panel aside-section">
            ...
            {loading ? (
              <div className="dash-loading" style={{ marginTop: '1rem' }}>
                ⟳ Loading...
              </div>
            ) : sweeps.length === 0 ? (
        ...
        {activeSweep ? (
                ...
                <div>
                  <h2 style={{ margin: 0 }}>
                    {getBestTrial(activeSweep)
                      ? `🏆 Best Trial: ${getBestTrial(activeSweep)?.trial_id}`
                      : 'No completed trials yet'}
                  </h2>
                  <span style={{ fontSize: '0.85rem', opacity: 0.7 }}>
                    {activeSweep.name} · {activeSweep.strategy}
                  </span>
                </div>
                <span
                  style={{
                    fontSize: '0.8rem',
                    color: statusColor(activeSweep.status),
                    textTransform: 'capitalize',
                  }}
                >
                  ● {activeSweep.status}
              ...
              {getBestTrial(activeSweep) ? (
                ...
                >
                  {Object.entries(getBestTrial(activeSweep)?.params ?? {}).map(([key, val]) => (
                    ...
                    >
                      <div style={{ fontSize: '0.75rem', opacity: 0.6, marginBottom: '0.2rem' }}>
                        {key.replace('_', ' ')}
                      </div>
                    ...
                    >
                      <td
                        style={{
                          ...
                          color: idx === 0 ? 'var(--accent)' : 'inherit',
                        }}
                      >
                        {idx === 0 ? '🏆 1' : idx + 1}
                      </td>
                      ...
                      <td style={{ padding: '0.6rem 0.75rem' }}>
                        {(trial.metrics.accuracy * 100).toFixed(1)}%
                      </td>
                      ...
                      <td style={{ padding: '0.6rem 0.75rem', opacity: 0.7 }}>
                        {trial.duration_s}s
                      </td>
                    </tr>
                  ))}
          ...
          >
            ...
            <p style={{ opacity: 0.45 }}>
              Launch a sweep using the form, or select one from the history panel.
            </p>
          </div>
        )}
      </div>
